...
Complejidad: Este comando debe ejecutar en \mathcal{O}(C^n)O(C 
n
 ), pero realizando una buena poda puede reducirse sustancialmente el tiempo de ejecución.
Ejemplo: Entrada:
...
Lo cual quiere decir: por cada par de adyacentes al vértice en cuestión, si existe la arista yendo de uno al otro (si además está la recíproca, lo contamos otra vez). A esa cantidad de aristas lo dividimos por k_i(k_i - 1)/2k 
i
​	
 (k 
i
​	
 −1)/2 (de allí el 2 multiplicando) siendo k_ik 
i
​	
  el grado de salida del vértice ii. En caso de tener menos de 2 adyacentes, se define que el coeficiente de clustering de dicho vértice es 0. Considerar que el coeficiente de clustering es siempre un número entre 0 y 1.
...
Complejidad: En caso que se indique una página en particular, debe ejecutar en ~ \mathcal{O}(1) O(1) (considerando que la red es muy dispersa). En caso que no se indique ninguna página, deberá ejecutar en \mathcal{O}((C + L)^2)O((C+L) 
2
 ).
Ejemplo: Entrada:
